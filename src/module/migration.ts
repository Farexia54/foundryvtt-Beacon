...
import { BeaconActor, BeaconNpcData } from "./actor/Beacon-actor";
...
import { BeaconTokenDocument } from "./token";

let lp = Beacon.log_prefix;
...
export const migrateWorld = async function () {
  ui.notifications.info(
    `Applying Beacon System Migration for version ${game.system.data.version}. Please be patient and do not close your game or shut down your server.`,
    { permanent: true }
  );
  ...
  if ((await game.settings.get(game.system.id, Beacon.setting_core_data)) === core_update) {
    ...
    new Dialog(
      {
        ...
        content: `
<h1>Beacon 1.0 Migration - The Big One!</h1>
<div class="desc-text">
...
<p>The Beacon system has undergone a huge overhaul since the 0.1.x versions, including changing nearly all
...
<p>Beacon compendiums have been successfully migrated to core version ${core_update}.
Migration of NPCs, Deployables, and Tokens is continuing in the background, <b>do not log off or close the game</b>
until you see the notification "Beacon System Migration to version ${game.system.data.version} completed".</p>
...
<p>Beacon 1.0+ has moved to a model of automatically calculating NPC stats, similar to how Comp/Con does. 
...
on them in the Actors sidebar and clicking "Migrate Pilot". </p>`,
    ...
    new Dialog({
      ...
      content: `
<p>Something went wrong while attempting to build the core data Compendiums for the new Beacon system.
Please refresh the page to try again.</p>`,
  ...
  ui.notifications.info(`Beacon System Migration to version ${game.system.data.version} completed!`, {
    permanent: true,
  });
};
  ...
  for (let a of game.actors.values()) {
    try {
      if (a.data.type === EntryType.PILOT) {
        ...
        if (ret) {
          console.log(`== Migrating Actor ${a.name}`);
          await (a as BeaconActor).importCC(ret, true);
          console.log(ret);
          count++;
        }
      }
...
export const migrateActorData = async function (actor: Actor, minor: boolean = false) {
  ...
  const updateData: BeaconNpcData = { _id: origData._id, data: {} };
  ...
  const items = [];
  const ai = actor.items.contents as Array<BeaconItem>;
...
function cleanActorData(actorData: ActorData) {
  ...
  // Scrub system flags
  const allowedFlags = CONFIG.Beacon.allowedActorFlags.reduce((obj, f) => {
    obj[f] = null;
    return obj;
  }, {});
  if (actorData.flags.Beacon) {
    actorData.flags.Beacon = filterObject(actorData.flags.Beacon, allowedFlags);
  }
 ...
 * @param item
 */
export const migrateItemData = async function (item: BeaconItem<NpcClass | NpcTemplate | NpcFeature>) {
...
export const migrateSceneData = async function (scene, minor: boolean = false) {
  ...
  const tokens = scene.tokens.contents as Array<BeaconTokenDocument>;
...
export const migrateTokenData = async (token: BeaconTokenDocument) => {
  let updateData = {};
